...
LoadFunctionDefMMA[head_Symbol] := Module[
	...
	downValues = DownValues[head]//.head->iHead;
	
	
	DownValuesRule = downValues//.{
		RuleDelayed->Rule, HoldPattern->Identity
	};
	
	upValues = UpValues@head//.head->iHead;
	
	UpValuesRule = MapThread[
		TagRule, 
		{
			ConstantArray[head, Length@upValues],
			upValues[[All, 1]], 
			upValues[[All, 2]]
		}
	...
	upValues/.(RuleDelayed[HoldPattern[$D[{n__}, x_Symbol]], y_]) -> TagRule[x, $D[{n}, x], y];
	
	allRules = Join[DownValuesRule, UpValuesRule];
...
DerivativeRulesLoadMMA[theory_String, hphcHead_String] := Module[
	...
	Definitions = Get[MMADirec <> "/Defs.mx"];
	
	
	hphc = ToExpression[hphcHead];
